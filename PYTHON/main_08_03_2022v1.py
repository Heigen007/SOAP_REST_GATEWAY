...
def getRequestGetList(self, id):
    try:
        ...
        if(json is None or 'materialMovementRequests' not in json):
            ...
            return generateErrorXml(self, "От сервера СУДМ 10.8.4.244:8010 на вызов GET-метода /api/material-movement/v1.0/stock получен пустой список materialMovementRequests",False, 0)
        ...
        for item in json['materialMovementRequests']:
            ...
            xml += "<n:v>"
            xml += "<n:id>" + ifnull(item['id']) + "</n:id>"
            xml += "<n:stateName>" + ifnull(item['stateName']) + "</n:stateName>"
            xml += "<n:senderStockName>" + ifnull(item['senderStockName']) + "</n:senderStockName>"
            xml += "<n:stateUpdateDate>" + ifnull(item['createDate']) + "</n:stateUpdateDate>"
            xml += "<n:stateName_and_updateDate>" + ifnull(item['stateName']) + ", " + str(dateTime) + "</n:stateName_and_updateDate>"
            xml += "</n:v>"
        xml += "</n:GetListResponse>"
        xml += "</s:Body>"
        xml += "</s:Envelope>"
...
def getRequestGet(self, id):
    try:
        ...
        for item in json['materialMovementRequests']:
            xml += "<n:id>" + ifnull(item['id']) + "</n:id>"
            xml += "<n:stateId>" + ifnull(item['stateId']) + "</n:stateId>"
            xml += "<n:stateName>" + ifnull(item['stateId']) + "</n:stateName>"
            xml += "<n:senderStockId>" + ifnull(item['senderStockId']) + "</n:senderStockId>"
            xml += "<n:senderStockName>" + ifnull(item['senderStockId']) + "</n:senderStockName>"
            xml += "<n:receiverStockId>" + ifnull(item['receiverStockId']) + "</n:receiverStockId>"
            xml += "<n:createDate>" + ifnull(item['createDate']) + "</n:createDate>"
            xml += "<n:stateUpdateDate>" + ifnull(item['createDate']) + "</n:stateUpdateDate>"
            xml += "<n:rejectReasonId>" + ifnull(item['rejectReasonId']) + "</n:rejectReasonId>"
            xml += "<n:rejectReasonName>" + ifnull(item['rejectReasonId']) + "</n:rejectReasonName>"
            xml += "<n:note>" + ifnull(item['note']) + "</n:note>"
            for material in item['materials']:
                xml += "<n:materials>"
                xml += "<material_id>" + ifnull(material['materialId']) + "</material_id>"
                ...
                xml += "<volume>" + ifnull(material['volume']) + "</volume>"
                xml += "</n:materials>"
        xml += "</n:GetResponse>"
        xml += "</s:Body>"
        xml += "</s:Envelope>"
