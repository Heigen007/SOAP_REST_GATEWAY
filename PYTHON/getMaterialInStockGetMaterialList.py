...
def getMaterialInStockGetMaterialList(self, id, typeId):
    try:
        ...
        for item in json['materialsInStock']:
            if (item.get('materialTypeId') and item.get('materialTypeId') == typeId):
                xml += "<n:v>"
        ...
        return generateErrorXml(self, "Cистемная ошибка в коде реализации сервиса /api/hobo-bff/v0.5/MMMS_Material_In_Stock.GetMaterialList: " + str(e), False, 0)